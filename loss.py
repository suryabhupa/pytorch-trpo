...
def compute_gradient_estimates(batch, policy_net, value_net, args, num_eval_grad_steps, grads_list, writer):
    ...
    new_returns = [0, 0, 0, 0]
    # i represents the k step returns
    new_returns[0] = returns
    for i in range(1, 4):
        new_returns[i] = returns - values.data
        new_returns[i] = new_returns[i].view(-1, ep_len)
        new_returns[i] = torch.cat([new_returns[i][:,i:], torch.zeros(new_returns[i].size(0), i)], 1)
        new_returns[i] = new_returns[i].view(-1, 1)
        new_returns[i] = new_returns[i] * (args.gamma ** i)

        new_targets = Variable(new_returns[i])
